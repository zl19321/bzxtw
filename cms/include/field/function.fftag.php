...
function smarty_function_fftag($params, &$smarty) {
	...
	$_tag = D("Tag");
	$_tc  = D("ContentTag");
	//select ft.name from %DB_PREFIX%_tag  ft left join 
	if (!isset($tag)) {
		$tag = $_tag ->field("%DB_PREFIX%_tag.name")-> join("%DB_PREFIX%_content_tag  on %DB_PREFIX%_tag.tagid= %DB_PREFIX%_content_tag.tagid")->where(" %DB_PREFIX%_content_tag.keyid= concat('c-',".$cid.")")->findAll();
		foreach ($tag as $t){
			$tag_array[] = $t['name'];
		}
	}
	...
	if (!empty($c_id)) {
		...
		$sql = "select * from %DB_PREFIX%_content where  cid in ($query)  ".$where ." order by cid DESC limit 0,".$pagesize;
