...
class View extends Think
{
    ...
    protected function output($content,$display) {
        ...
        if($display) {
            if(C('SHOW_RUN_TIME')){
                $runtime = '<div  id="think_run_time" class="think_run_time" style="display:none;">'.$this->showTime().'</div>';
    ...
    protected function templateContentReplace($content) {
        // 系统默认的特殊变量替换
        $replace =  array(
//            '../public'   => APP_PUBLIC_PATH,// 项目公共目录，由于编辑器里的文件直接上传的文件产生的链接都是  ../public/uploads/*.*  所以该字符串不能替换
