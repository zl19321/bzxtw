...
class HtmlCache extends Think {
	private static $cacheTime = null; // 缓存有效期（支持函数）
	...
	static private function requireHtmlCache() {
		...
		if (! empty ( $htmls )) {
			...
			// 检测静态规则，检测顺序：全局 > 空模块 > 具体模块。后如果有重复，则后面的覆盖前面的
			
			if (isset ( $htmls ['*'] )) {					
				$html = $htmls ['*']; // 全局静态规则
			}
			if (isset ( $htmls ['Empty:index'] ) && ! class_exists ( MODULE_NAME . 'Action' )) {
				$html = $htmls ['Empty:index']; // 空模块静态规则
			}
			if (isset ( $htmls [MODULE_NAME . ':_empty'] ) && self::isEmptyAction ( MODULE_NAME, ACTION_NAME )) {
				$html = $htmls [MODULE_NAME . ':_empty']; // 空操作静态规则
			}			
			if (isset ( $htmls [MODULE_NAME . ':'] )) { // 某个模块的静态规则
				$html = $htmls [MODULE_NAME . ':'];
			}
			if (isset ( $htmls [ACTION_NAME] )) {
			...
			if (! empty ( $html )) {
				...
				$rule = str_ireplace ( array (
					...
					APP_NAME, MODULE_NAME, ACTION_NAME, GROUP_NAME 
				), $rule );
				// {|FUN} 单独使用函数
