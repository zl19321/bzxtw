 ...
 * @version   $Id$
 +------------------------------------------------------------------------------
 */
class Dir extends Think implements IteratorAggregate { //类定义开始
...
/**
	...
	function clearDir($directory) {
		...
		while ( ($file = readdir ( $handle )) !== false ) {
			if ($file != "." && $file != "..") {
				is_dir ( "$directory/$file" ) ? Dir::clearDir ( "$directory/$file" ) : unlink ( "$directory/$file" );
			}
		}
		if (readdir ( $handle ) == false) {
			closedir ( $handle );
	 ...
	 * @return void
	 +----------------------------------------------------------
	 */
	function delByTime($directory,$time = 3600) {
		if (is_dir ( $directory ) == false) {
			return false;
		}
		$handle = opendir ( $directory );
		while ( ($file = readdir ( $handle )) !== false ) {
			if ($file != "." && $file != ".." && is_file ( "$directory/$file" )) {
				if (time()-filemtime("$directory/$file") > $time) {
					@unlink ( "$directory/$file" );
				}				
			}
		}
