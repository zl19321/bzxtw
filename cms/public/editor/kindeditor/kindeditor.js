...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'advtable', 'hr', 'emoticons', 'link', 'unlink', '|', 'pagebreak'
	],
	colorTable : [
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						...
						src : data.url,
						type : KE.g[id].mediaTypes[KE.util.getMediaType(data.url)],
						width : width,
						height : height,
						autostart : autostart,
						loop : 'true'
...
KE.lang['pagebreak'] = "分页符";
KE.plugin['pagebreak'] = {
    click : function(id) {
        KE.util.insertHtml(id,'<div class="ke-pagebreak"></div>');
    }
};
