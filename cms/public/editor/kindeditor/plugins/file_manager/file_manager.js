var JSON_URL = '../../../../../admin.php?m=feditor&a=manage&type=kind';
var KE = parent.KindEditor;
location.href.match(/\?id=([\w-]+).*\&uploadfiletype=([\w-]+)/i);
var id = RegExp.$1;
var uploadfiletype = RegExp.$2;
...
KE.event.ready(function() {
	...
	var httpRequest = function (param, func) {
		...
		if(reg.test(url)){
			...
			url += (reg.test(url) ? "&" : "?") + param;
		}else{
			url += param;
		}
		url += (reg.test(url) ? "&" : "?") + 'uploadfiletype=' + uploadfiletype;
		url += (reg.test(url) ? "&" : "?") + (new Date()).getTime();
		//url += param;
