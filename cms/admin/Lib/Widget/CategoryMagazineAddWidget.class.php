...
class CategoryMagazineAddWidget extends Widget {
	...
	public function render($data) {
		...
		$model_data = F ('model_'.$data['modelid']);
		if (empty($setting ['template']['index'])) {
			$data['setting'] ['template']['index'] = 'magazine/index.html';
		}
		if (empty($setting ['template']['lists'])) {
			$data['setting'] ['template']['lists'] = 'magazine/lists.html';
		}
		if (empty($setting ['template']['view'])) {
			$data['setting'] ['template']['view'] = 'magazine/view.html';
		}
		if (empty($setting ['template']['show'])) {
			$data['setting'] ['template']['show'] = 'magazine/show.html';
		}
...
//		dump($permissions);
		...
		$html ['body'] = '
			<table cellpadding="0" cellspacing="0" class="tabcontent" id="tabsContent3" style="display:none">
				<tbody>
				<tr>
		          <th width="150">频道页模板<span>该栏目首页所使用的模板</span></th>
		          <td>' . Html::template ( 'info[setting][template][index]', $setting ['template']['index'],'class="input" title="请选择频道页模板"' ) . '</td>
		      	</tr>
		      	<tr>
		          <th width="150">列表页模板<span>该栏目列表页所使用的模板</span></th>
		          <td>' . Html::template ( 'info[setting][template][lists]', $setting ['template']['lists'],'class="input" title="请选择列表页模板"' ) . '</td>
		      	</tr>
		      	<tr>
		          <th width="150">内容页模板<span>栏目下详细内容显示所使用的模板</span></th>
		          <td>' . Html::template ( 'info[setting][template][view]', $setting ['template']['view'], 'class="input" title="请选择内容页模板"' ) . '</td>
		      	</tr>
		      	<tr>
		          <th width="150">文章内容页模板<span>栏目下文章内容页显示所使用的模板</span></th>
		          <td>' . Html::template ( 'info[setting][template][show]', $setting ['template']['show'], 'class="input" title="请选择内容页模板"' ) . '</td>
		      	</tr>
		      	<tr>
