...
class CategoryGuestbookAddWidget extends Widget {
...
//		dump($permissions);
		...
		<table cellpadding="0" cellspacing="0" class="tabcontent" id="tabsContent3" style="display:none">
				...
				<tr>
		          ...
		          <td>' . Html::template ( 'info[setting][template][index]', $setting ['template']['index'] ? $setting ['template']['index'] : 'guestbook/index.html','class="required input" title="请选择列表页模板"' ) . '</td>
		      	</tr>
		      	<tr>
		          <th>留言 页模板<span>填写留言的页面模板，默认空</span></th>
		          <td>' . Html::template ( 'info[setting][template][show]', $setting ['template']['show'] ? $setting ['template']['show'] : 'guestbook/submit.html', 'class="input" title="请选择留言 页模板"' ) . '</td>
		      	</tr>
		      	<tr>
		          <th>是否留言必须登录</th>
		      	...
		      	<tr>
		          <th>是否留言必须审核</th>
		          <td><input type="radio" name="info[setting][isstatus]" value=1 '.($setting['isstatus']==1?'checked':'').' />是 <input type="radio" name="info[setting][isstatus]" value=0 '.($setting['isstatus']!=1?'checked':'').' />否</td>
		      	</tr>
				<tr>
