...
class ActsModel extends Model {
	...
	/**
	 * ajax方式保存授权，gaohuazi新增批量操作   
	 */
	
	public function ajaxSaveAll($data) {

		if (empty($data['do']) || !$data['role_id'] || empty($data['appname'])) return null;
		//获取角色信息
		$role_data = D('Role')->find((int)$data['role_id']);
		$pre=C('DB_PREFIX');
		if(is_array($role_data)) {
			if($data['do'] == 'add'){
				$sql1="update {$pre}acts set allow=CONCAT(allow,',".$role_data['name']."') where appname='".$data['appname']."' and allow is not null";	
				$sql2="update {$pre}acts set allow=',".$role_data['name']."' where appname='".$data['appname']."' and allow is null";	
				// file_put_contents('fuck.txt', $sql1."\r\n".$sql2);
				$b1=M()->execute($sql1);
				$b2=M()->execute($sql2);
				...
				return false;;
				// return true;	
			}elseif($data['do'] == 'remove'){
				$sql1="update {$pre}acts set allow=REPLACE(allow,',".$role_data['name']."','') where appname='".$data['appname']."'";
				// file_put_contents('fuck.txt', $sql1."\r\n");
				return M()->execute($sql1);
			}
